...
-   Filter data to only consider "Overall Health"

-   Select to keep the following variables: `year`, `location_abbr` which includes an abbreviation for state, `location_desc` which includes a description of specific site, `response` which includes description of response for survey, and `data_value` which includes the numeric value of response.
...
-   **How many unique locations are included in the dataset? Is every state represented? What state is observed the most?** There are 404 unique locations included in the dataset. All 51 states are represented including Washington, DC. \#\# get state observed the most
