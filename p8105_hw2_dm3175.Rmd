...
To finish making this data tidy, data (both original clean `NYC_traffic_data` dataset and `distinct_NYC_traffic_data` distinct stations datasets) are reformatted so that route number and route name are distinct variables. Route names equal to "NA" were removed, and data was sorted by line and station_name. 
...
There are `r filter(tidy_distinct_NYC_traffic_data, route_name == "A") %>% nrow()` distinct stations serving the A train.
